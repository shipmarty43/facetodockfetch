...
Используйте специальный скрипт для запуска без Conda:

```bash
cd /home/user/facetodockfetch
./scripts/quick_start_no_conda.sh
```
...
- Запустит backend на порту 30000
- Запустит frontend (если установлен npm)

После запуска откройте http://localhost:3003 и войдите с **admin** / **admin123**
